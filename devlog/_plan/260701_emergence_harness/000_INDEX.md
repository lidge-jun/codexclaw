...
| `007_collapse_point_model.md` | The collapse-point model: I keeps the N>=2 recording rule (asymmetric "strong-1 + add-1" OK); the USER-FACING question is conditional (clear intent → converge silently, don't interrogate; open intent → ask); build/satisfy-spec collapses EARLY at P; algo/unclear collapses LATE — parallel A-B-C then race at D on the fixed-seed metric. |
