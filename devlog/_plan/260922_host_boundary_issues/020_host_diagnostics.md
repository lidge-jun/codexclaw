...
MODIFY components/recall/test/memory-status.test.ts and affected typed literal fixtures (enumerate by type/field searches before edit). Creation is collectMemoryStatus -> direct JSON serialization in CLI -> no persisted deserialization -> formatter/notice consumers. Existing callers are compatibility consumers and must preserve their prior behavior. Tests activate empty readable DB, success history, quota-classified failure, absent/unsupported DB: observability stays unknown in all. Verify existing memory search chat fallback without adding another fallback.

MODIFY skills/recall/SKILL.md status documentation and structure/INDEX.md to distinguish partial improvements from native fixes. #209/#191 remain open. Verifiers: node plugins/codexclaw/scripts/test.mjs plugins/codexclaw/test/lane-packet.test.mjs plugins/codexclaw/components/recall/test/memory-status.test.ts plus directly affected fixture tests; npm run build; gate. Independent review confirms no optimistic route or ID guesses.


## wp2 P decisions after source revalidation

Architect 01a0c916-9b3f-7710-9d59-13df74206475 D1-D6 accepted. Legacy no-mode packets without creation retain address-based dispatch/bound defaults. A present malformed/empty mode fails; creation requires explicit pending/bound mode (packet or CLI). pending forbids address property entirely, dispatch rejects creation and any provisional address fields; bound validates optional creation and rejects copying either recorded provisional id. Define requestedAt as canonical UTC ISO 8601 YYYY-MM-DDTHH:mm:ss[.sss]Z, validate by round-trip calendar comparison (invalid normalized dates fail). Optional worktree must be a nonempty string. No host inference from time/cwd/title.

CLI parser accepts options before/after exactly one file, rejects missing or repeated --mode, unknown flags/modes and extra positional inputs; explicit CLI/declaration conflict always fails. Single result includes resolved.mode and text prints it. Packet-set response shape and collision validation remain unchanged; test mixed pending/bound sets.

Memory observationSource denotes the jobs-db collector, not successful DB access. Actual consumers are runMemoryStatus JSON/text and memoryStatusNotice SessionStart; doctor is not a current consumer. Tests cover corrupt DB plus every existing state, actual CLI JSON/text/exit status and unchanged healthy startup silence. Existing recall fallback remains unchanged. Focused baseline was 40/40; changed tests directly observe the edited contracts.
