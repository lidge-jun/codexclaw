...
export interface Assumption {
  ...
  severity: ContradictionSeverity;  // low/medium auto-recorded; high should have been escalated
...
export function isInterviewReady(tracker: InterviewTracker | null): boolean {
  ...
  // R-H: dimensions at max + every assumption explicitly recorded (NOT empty) + no UNRESOLVED
  // high-severity contradiction still pending. Assumptions are allowed; they ride into OPEN ASSUMPTIONS.
  return (
    dims.every((d) => d.level === "max") &&
    tracker.assumptions.every((a) => a.recorded) &&
    tracker.contradictions.every((c) => c.severity !== "high")
  );
}
```
...
| readiness 판정 | state | `state.ts` | `isInterviewReady()` | all-max + 모든 assumption recorded + no pending high contradiction (R-H) |
...
const INTERVIEW_PROTOCOL = [
  ...
  "Interview is ready when all dimensions reach 'max', every assumption is RECORDED into the spec's OPEN ASSUMPTIONS section, and no high-severity contradiction is still pending (R-H — assumptions need not be empty).",
  "",
...
test("isInterviewReady: all max + low/medium contradictions still allowed if recorded -> true")
...
- → §1 `isInterviewReady()` 수정: 4차원 점수 충족 + assumptions가 (비어있거나) 전부 명시 기록됨.
...
- A(audit): 각 open assumption을 리스크로 명시 검토. C(check): 가능하면 테스트로 assumption 도전.
  D(done): 미해결 assumption을 결과 보고에 명시. → "투명 명시"가 실행 단계에서 실제로 constrain되게.
