...
- `memory-search.ts` production code needs no structural edit if it keeps calling `normalizeCwd` + `cwdMatches(..., { caseInsensitive: FOLD_CWD_CASE })`. Do not pre-edit it for L2.
...
```ts
/** SQL expression mirroring normalizeCwd. `expr` is a column or `?`. */
...
Lock with a `node:sqlite` `:memory:` parity loop: for every fixture input, `SELECT ${canonicalCwdSql("?")} AS n` equals `normalizeCwd(input)`.
...
/** SQL expression mirroring `normalizeCwd`. `expr` is a column name or `?`. */
export function canonicalCwdSql(expr: string): string {
  const slashed = `rtrim(replace(${expr}, '\\', '/'), '/')`;
  const stripped = `(CASE WHEN substr(${slashed}, 1, 8) LIKE '//?/UNC/' THEN '//' || substr(${slashed}, 9) WHEN substr(${slashed}, 1, 4) = '//?/' THEN substr(${slashed}, 5) ELSE ${slashed} END)`;
  const trimmed = `rtrim(${stripped}, '/')`;
...
### 5.4 `plugins/codexclaw/components/recall/src/memory-search.ts` — no production edit

Already wired. Confirm after the `rollout.ts` change that `cwd-scope.test.ts` existing Windows separator tests (`:375-421`) still pass, and add the `\\?\` / `repo_key` NULL cases in §6.1. Do not edit this file just to make L2's later patch look shared.
...
Input: the same rows, plus `""`. Open `new DatabaseSync(":memory:")` (already imported at `:13`). For each input `q`, `db.prepare(`SELECT ${canonicalCwdSql("?")} AS n`).get(q).n === normalizeCwd(q)`.

**Why it fails today:** `canonicalCwdSql` does not exist (implementer creates it in §5.1). After it exists, any JS/SQL drift fails this test.
...
Expected: all four calls return exactly 1 hit; `hit.cwd` remains the raw recorded string `\\\\?\\C:\\Users\\super\\Developers` (ingest is not rewritten); a sibling `cwd: "C:\\Users\\super\\Developers2"` returns 0 on both engines.

**Why it fails today:** scan with `C:/Users/...` already slash-folds but does not strip `\\?\`, so scan is also 0 against this recorded cwd — that is bug (a). Index is 0 for slash-query even against a backslash-recorded cwd (bug (b)). After the fix both engines return 1. On parent tip, assert the failure mode: index+slash is 0; scan+either is 0 against `\\?\` recorded cwd.
...
Expected: index and scan both 1 for both spellings.
...
**Why it fails today:** on win32, `FOLD_CWD_CASE` is false (`rollout.ts:114`), so both engines miss (scan via `cwdMatches` without fold; index via byte-exact `f.cwd = ?`). After the fix, win32 hits on both. On linux, still miss on both — the test's `FOLD_CWD_CASE` branch documents that.
...
L2 (`020_wp3_memory_search_semantics.md`, #142+#143) owns the next `memory-search.ts` production edit (blank-line AND, `matchedThreadIds`, chat-fallback `synonyms`/`any`). This layer only adds tests in `cwd-scope.test.ts`.
