...
SessionStart now injects a subagent dispatch card, even when no role has a fallback. With `CODEXCLAW_SPAWN_V1=1` (labeled an override, not host evidence) it prints the exact `tools.multi_agent_v1__*` calls. Otherwise it prints a single Code Mode cell that finds the unique `spawn_agent` helper, identifies V1 or V2 from companion tools, and spawns (V2 with `task_name` and `fork_turns: "none"`). It stops before calling if the helper or the family is ambiguous. A dated alias map (deepseek, swe2, kimi, sol, luna) is marked verified or unverified against the local Codex catalog, and full ids pass through unchanged. `subagents_get` adds `spawnArgs`, plus a `staleModel` of true, false or null with a `staleReason`, from a 5 s bounded fresh catalog read. The MCP stdio server now serializes requests, so replies keep their order and none are lost at EOF. Commits a1a47c30 and bf7552b7. Evidence: full npm test 3604/3609 (0 fail) under the wp7 receipt. Host smoke: the card's cell, pasted verbatim, spawned command-code/deepseek in one Code Mode cell twice (agents 01a0d277, 01a0d27d answered deepseek-v4.1-flash). Reviewer 01a0d277-d726 returned FAIL (V2 full-fork default, lost MCP replies, untracked dist, namespace-bound resolver, overstated docs), then PASS.

What did not improve: V2 is proven only against a tool harness, not a live V2 host. The issue's literal "no ALL_TOOLS scan" is met as "the spawn happens in the first cell", because SessionStart carries no catalog. Hypothesis that died: that a namespace substring identifies the V2 family; flat names made companion tools necessary.
