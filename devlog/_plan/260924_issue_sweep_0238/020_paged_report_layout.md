...
The paged-report template now sizes the contents label column from its content. It carries a translator comment on the `@page` title and date literals and paints its chart grid before the labels. DIAGRAM-LAYOUT-01 states the paint-order and halo rule. The exporter adds a P2 finding when a non-Korean document keeps Hangul or a Korean-format date in `@page` content; the scan ignores CSS comments but keeps comment markers inside strings. It also runs a bounded `--dump-dom` pass on the final filled HTML that reports a label crossed by a later-painted connector as P2. That pass honors each connector's own transform and hidden or transparent ancestors, and any failure of the pass is a nonblocking note. Commits: f3402ba1, 81fd5863, 48aa32bb. Evidence: 148/148 with the real-Chrome smoke under the wp3 receipt. Independent reviewer 01a0d1ce found three medium defects, then one more in the fix, then PASS; each fix had a red-green test.

What did not improve: the SVG check samples geometry, so a narrow crossing between samples or overlap with glyph whitespace can still be missed or over-reported, and rendered-page inspection stays the authority. The locale lint only sees `<style>` blocks, not external stylesheets. Hypothesis that died: that the root SVG's screen matrix was enough for connector points; group transforms broke it. What would show this direction is wrong: real reports where most P2 crossings are false positives. That would argue for moving the check behind an explicit flag, which this phase deliberately did not add.

Next: wp4 from 030_goalplan_cli_flags.md.
