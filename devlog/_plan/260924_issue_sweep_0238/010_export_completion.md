...
Current anchors: `killToolTree` at lines 89–106, `runTool` at 108–146, `printPdf` at 183–197, `finish` at 302–347, first print dispatch in `main` at 393–400, second print dispatch at 455–464, and CLI entry at 511–512.
...
Replace `runTool` at current lines 108–146 with the same subprocess behavior plus an optional completion probe. The following is the required behavior and replacement body. `completionPath` is passed only by `printPdf`; `killTree` and `postKillGraceMs` are dependency injection used by the unit-level cleanup test.
...
async function runTool(tool, args, timeoutMs, {
  ...
  try {
    ...
    result = await new Promise((resolveResult) => {
      ...
      const requestKill = (reason) => {
        ...
        if (child.pid) {
          try { cleanupError = killTree(child); }
          catch (error) { cleanupError = `tree cleanup failed: ${error.message}`; }
        }
      ...
      child.once("exit", (status, signal) => finishResult({
        status,
        signal,
        timedOut,
        completionRequested,
        cleanupError,
        completedBy: completionRequested === "stage-stable" ? "stage-stable" : "exit",
      }));
  ...
  if (result.completionRequested === "stage-stable") return { ok: true, stdout: result.stdout || "", completedBy: "stage-stable" };
...
The implementation must preserve the current deadline semantics: a 100 ms deadline cannot satisfy the 1,500 ms stability window. A nonzero exit before the probe requests cleanup remains a failure. A stage that has completed and is deliberately killed by the probe is successful only if the cleanup call reports success and the child exits before the 5,000 ms grace period.
...
5. `runTool reports cleanup failure when injected kill cannot produce child exit` — call imported `runTool` with a short-lived hanging Node child, `timeoutMs` 100, `postKillGraceMs` 20, and a `killTree` injection that captures the PID and returns `tree cleanup failed: injected` without killing it; assert `ok === false`, the reason contains the cleanup text and post-kill grace failure, and kill the captured PID in `finally`.
6. `human summaries print each failed check id and reason once` — omit `--json`, run an existing timeout mode, assert exit 1 and that stdout contains exactly one `FAIL artifact-created:` line with the timeout reason. Also run the missing-Poppler case and assert each `NOT_RUN <id>:` reason is named once.
...
5 seconds before promoting the stage; cleanup failure is a failed export. Verify
...
| --- | --- |
| Creation | `runTool` returns `completedBy`; `printPdf` preserves it; `main` creates `report.printPasses` and appends one object after each successful print at the current first-pass and second-pass call sites. |
| Serialization | `finish` serializes the report through the existing `JSON.stringify(report, null, 2)` branch at current `export-paged-report.mjs:338`; human output reads the same in-memory field only for the pass count and reasons. No new file format is introduced. |
...
| Compatibility | Keep `schemaVersion: 1`; the field is additive. Consumers that ignore unknown fields remain valid. `completedBy` is an internal string union, not a public enum export. |

## C-ACTIVATION-GROUNDING-01

| Conditional path | Activation scenario C will run | Observable effect |
| --- | --- | --- |
| Stable-stage completion | Fixture mode `complete-then-hang`, first print; valid header/trailer, unchanged size/mtime for 1.5 s, Chromium remains alive. | `runTool` requests tree cleanup, the child exits within grace, CLI exits 0, stage is promoted, and `printPasses[0].completedBy` is `stage-stable`. |
...
| Nonzero race | Fixture mode `complete-then-nonzero`; complete bytes are written immediately before exit 8. | The exit status is retained as failure; a complete-looking stage is not promoted. |
...
| Post-kill cleanup and grace | E8, script-level gate | `killToolTree`, `runTool`, and the 5-second grace timer | An alternative exporter or a caller that ignores the exporter result bypasses it; OS-level descendants can remain beyond what the parent/taskkill result proves. | The phase proves the owned child and cleanup command outcome, not universal descendant disappearance. | Say “promotion fails when cleanup is reported unsuccessful or the child does not exit”; do not claim a complete OS sandbox. |
...
| `node plugins/codexclaw/scripts/test.mjs "plugins/codexclaw/test/report-export.test.mjs"` after implementation | NOT RUN (new). | Will read the modified exporter, fixture, and every new test, including stable first/second pass, changing content, injected cleanup failure, and human summary assertions. |
...
| --- | --- | --- |
| `plugins/codexclaw/skills/dev-visualizer/reference/report-pipeline.md` | 116–121 | MODIFY exactly as shown in the file map: document the 1.5-second header/trailer/size/mtime probe, 5-second cleanup grace, failed promotion, per-pass completion method, and unchanged `--qa-only` path. |
...
Open decisions: none. D2.1, D2.2, and D2.3 settle the intended behavior; the unit-level injection is the explicit test method for the uncatchable SIGKILL/no-exit case.
