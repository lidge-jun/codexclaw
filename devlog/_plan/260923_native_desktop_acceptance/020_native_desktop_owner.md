...
1. §1 Scope and triggers (DESKTOP-SCOPE-01). Load for Tauri, AppKit/SwiftUI, WidgetKit, menu-bar/tray, native window effects, embedded runtimes/sidecars, or desktop release readiness. A Swift/Rust change that alters a presented native surface triggers UI rows; a non-UI model change triggers runtime and packaging rows only, never blanket screenshot demands. Register goalplan criteria for these rows on a session-bound plan with `cxc loop add-criterion --session <id> --surface desktop` (0.2.37+; `init` rejects `--surface`). Without a recorded final gate that is a classification the agent must honor by producing the matrix; validation on schemaVersion 2+ plans with a final gate, and the final-gate spawn guard on any plan with a recorded finalGate, also make the QA receipt mandatory. Hosts that edit the plan need 0.2.37 or newer, because older builds erase the value.
