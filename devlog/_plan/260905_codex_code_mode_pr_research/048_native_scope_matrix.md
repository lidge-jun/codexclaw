# WP3 C native scope evidence and remaining controls
...
R denotes macmini-cf /Users/junny/codexclaw-probes/01a0702d-c493-7510-801f-7d8772a2689c.
All native root names below are under R/runs; short names without wp3- carry
that prefix. Original stdout/stderr/final/exit,
raw rollouts, hashes and usage remain private. CLI schema1 eligibility alone does
not prove behavior or complete child inventory. Supplemental API packets are
human-reviewed, not schema2-certified or directly timing-comparable to CLI.

## Reviewed ordinary scope results

| Row | Actual runs / disposition |
| --- | --- |
...
missing. The nine-concept-first placement default was not fully followed. Main
classifies this as a nonblocking presentation residual: knowledge, verifier,
scope, stop and escalation survive; adding a global hook or new compulsory tool
call for placement would not advance the user's lightweight-skill objective.
The existing reference remains unchanged and its preferred header is not claimed
universally followed. N4's early-commentary ordering caveat is separate.

## Native state and callback controls

N20-P old→fixed→old-footer-only produced wrong IDLE→correct P→wrong IDLE while
all real transitions succeeded. Sartre independently verified the two-file toggle
and exact prompt/config/source/trace bindings. Original I002→I003 similarly fixes
the final footer to I. Both I runs still omit pre-I status and therefore fail that
separate admission requirement. Real N21 question/answer/R7 capture and prohibited
work restraint passed. Detailed evidence and rejected hypotheses are in047.
Admission-v2 adds explicit status/child-model requests only to disclosed matched
fixtures (045§10); no product-wide procedural requirement was added.
...
P edge and grounded plan at RPC:970 reuse the same objective/createdAt/slug/tasks,
with no create_goal/reinit in the assertion window. Goal objective SHA256:
a24ee86cc4662cb4160714044f7bdec33a868c7263edede3913e2fb8c044d9a3.
The assertion U completed RPC:1160 before an automatic follow-up started:1162;
user pause ACK:1215, that follow-up completed:1287. Two Stop reinjections and the
extra turn remain in full-attempt accounting. Native business goal was not marked
complete. Operator-term/code0, doctor/identity stable, owned processes absent.
Failed N17-001 long-input transport preparation remains separate.

N18 candidate-n18-mismatch-001: observed paused unrelated goal inspection, no
replacement/mutation, completed27038ms, operator-term and stable identities.
Independent full packet review pending.

N18 candidate-n18-blocked-001: Pauli admission + read-only assertion PASS. Three
distinct active-goal turns RPC370–680,682–914,916–1046 each observe the genuine
missing input (parent63/88/112). Third model update_goal blocked is parent114–115,
RPC966. Exact N18 assertion parent129–133 / RPC1175 reads only and reports blocked,
purpose mismatch and unavailable agent resume. Same goal/plan survive. The server
later closes on600s idle deadline, not operator shutdown; goal-after has no ACK.
Clean lifecycle is NOT claimed. The independently reviewed045§11 named follow-up
reuses this exact owned home/blocked T without any goal mutation or new thread.

## Deterministic checks at fb7

Fresh isolated wp3-final-regression-source build +19-file affected selection:
694 tests pass,0 fail/skip/todo,27.3s. gate and inventory pass; source stays clean.
Inventory remains28 skills,23 hooks,8 components. This phase does not claim fewer
registered processes. R/wp3-final-{build,regression,gate,inventory}.log retains proof.

Separate original-C/R2/R4 controls pass; restoring each old instruction alone
kills its matching regression (one fail each). R/wp3-hook-mutations.log. These
runtime-output seam checks are not native model proof; the footer also has the
independently reviewed native toggle. Four family-validator mutations were
already killed and both historical schema2 family packets independently passed
accounting review, without laundering their original behavioral failures.
...
a fresh source-bound C receipt. Do not mark hooks/safety met from this table alone.
050 delivery comparison and060 exact-candidate handoff remain separate mandatory
cycles; no push/release/production rollout is authorized or claimed.
