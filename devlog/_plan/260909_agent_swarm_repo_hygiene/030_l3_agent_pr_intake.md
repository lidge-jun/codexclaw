...
machine-style branch names (`codex/…`, `agent/<slug>-<date>`) pushed from human
accounts, and 56 were drafts.
...
| Opens the PR? | Yes, as **draft** | User clicks "Create Pull Request" | No — pushes a branch, returns a prefilled PR link |
...
| Instruction files | `.github/copilot-instructions.md`, `.github/instructions/**`, `AGENTS.md`, `CLAUDE.md` | `AGENTS.override.md` → `AGENTS.md`, root-down, 32 KiB cap | `CLAUDE.md` from the base branch; `AGENTS.md` unsupported |
...
- **Disclosure.** Godot, ghostty, LLVM, Kubernetes, OpenSSL require it; Crossplane
...
- **Is the flood permanent?** Godot, Ladybird and Zig treat it as structural. curl's
  ...
  bounty was removed — removing the incentive worked where banning the tool would not.
  Do not cite curl as proof that AI reports are worthless.
- **Ban the tool or cap the rate.** Zig, QEMU, Gentoo, Servo judge AI-ness. OpenSSL,
