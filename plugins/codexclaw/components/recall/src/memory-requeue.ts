...
import { openDbReadOnly, openDbReadWrite } from "./sqlite.ts";
import { classifyMemoryError } from "./memory-status.ts";
...
export interface RequeueResult {
  state: "ok" | "unavailable" | "unsupported";
  detail: string;
  storePath: string | null;
  /** True when rows were actually written. */
  applied: boolean;
  /** Rows that matched the selection. */
  selected: RequeueCandidate[];
  /** Exhausted rows deliberately left alone, by cause. */
  skippedByCause: Record<string, number>;
  /** Rows actually updated; 0 on a dry run. */
  changed: number;
  /** The retry allowance given to each selected row. */
  retries: number;
}

export interface RequeueOptions {
  /** Write. Without this the call only reports what it would do. */
  apply?: boolean;
  /** Also select context-window failures, which will most likely fail again. */
  includeContextWindow?: boolean;
  /** Restrict to this job kind. */
  kind?: string;
  /** Cap how many rows are touched. */
  limit?: number;
  /** Retry allowance to restore. */
  retries?: number;
}

const REQUIRED_COLUMNS = ["kind", "job_key", "status", "retry_remaining", "last_error"];

function shell(state: RequeueResult["state"], detail: string, storePath: string | null, retries: number): RequeueResult {
  return { state, detail, storePath, applied: false, selected: [], skippedByCause: {}, changed: 0, retries };
}

export function requeueExhaustedMemoryJobs(home: string, options: RequeueOptions = {}): RequeueResult {
  const retries = Number.isFinite(options.retries) && (options.retries as number) > 0 ? Math.floor(options.retries as number) : 3;
  ...
  let selected: RequeueCandidate[] = [];
  const skippedByCause: Record<string, number> = {};
  try {
    const columns = readDb.prepare("PRAGMA table_info(jobs)").all() as Array<{ name?: unknown }>;
    ...
    if (Number.isFinite(options.limit) && (options.limit as number) >= 0) selected = selected.slice(0, options.limit as number);
  } catch (err) {
  ...
  const base: RequeueResult = { state: "ok", detail: "", storePath, applied: false, selected, skippedByCause, changed: 0, retries };
...
export function formatRequeue(result: RequeueResult): string {
