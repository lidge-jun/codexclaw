...
export function createDiscordAdapter(opts                       )                 {
  ...
  function formatJobContext(bindingId        )         {
    const lines = opts.db
  ...
  function embedBatches(descriptions          )                   {
    const batches                   = [];
    let batch                 = [];
    let total = 0;
  ...
  async function sendCodeFenceEmbedReply(channelId        , text        )                   {
