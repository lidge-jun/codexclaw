...
function providerToCatalogInput(status                     )                          {
...
function getSubagentsRoute(cwd        )              {
  return { status: 200, body: readConfig(cwd) };
}

function postSubagentsRoute(cwd        , body         )              {
...
function getCatalogRoute()              {
  const status = detectOcx(detectDeps())                       ;
...
export function apiCompatRoutes()             {
  return [
    {
      method: "GET",
      path: "/api/subagents",
