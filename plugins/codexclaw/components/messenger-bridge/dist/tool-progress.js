...
export const TOOL_PROGRESS_MODES = ["off", "new", "all", "verbose"]         ;

export const DEFAULT_TOOL_PROGRESS                   = "new";
...
export function createToolProgressPolicy(mode                  )                     {
  const seen = new Set        ();
...
export function createToolProgressFilter(mode                  )                     {
  const policy = createToolProgressPolicy(mode);
  const filter = ((event             ) => policy.render(event))                      ;
