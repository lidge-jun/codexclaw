...
type VerbRule = {
  allowed: ReadonlySet<GoalplanFlag>;
  repeatable: ReadonlySet<GoalplanFlag>;
  usage: string;
};

const VERB_RULES: Readonly<Record<GoalplanVerb, VerbRule>> = {
...
export function parseGoalplanCliArgs(argv: string[], cwd: string): GoalplanCliArgs | GoalplanCliParseError {
  ...
  for (let i = 1; i < argv.length; i++) {
    ...
    const flag = (eq > 0 ? token.slice(0, eq) : token) as GoalplanFlag;
...
export function renderGoalplanHelp(): string {
  return [
    ...
    "",
    "Usage:",
    ...(["init", "show", "validate", "steer", "add-criterion", "add-work-phase", "ready", "add-task", "complete-task", "meet-criterion", "help"] as const)
