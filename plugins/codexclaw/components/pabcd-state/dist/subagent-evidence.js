...
function attemptsPath(cwd        , sessionId        , agentId        )         {
  return join(
    cwd,
    STATE_DIR,
...
export function readAttempts(cwd        , sessionId        , agentId        )         {
  try {
    ...
    if (parsed && typeof parsed === "object" && Number.isInteger((parsed                          ).attempts)) {
...
export function writeAttempts(cwd        , sessionId        , agentId        , attempts        )       {
  try {
    const p = attemptsPath(cwd, sessionId, agentId);
...
export function clearAttempts(cwd        , sessionId        , agentId        )       {
  try {
