...
import { hasNativeObservation, hasRenderArtifactModified, hasRenderObservation, nativeObservationLedgerMalformed, renderGroundingAdvisory } from "./render-observations.js";
 ...
 */
export function activeWorkPhaseOpts(cwd        , slug        )                                                                  {
 ...
 */
export function phaseDirective(phase       , opts                                                      )         {
  ...
  if (result.control === "done" && state.slug) {
    ...
    const locked = withGoalplanWriteLock(payload.cwd, state.slug, (plan) => {
      ...
      if (recoveringDclose) {
        ...
        if (closed.kind === "ok") {
          closeResult = { kind: "ok"         , closedId: closed.closedId, plan: closed.plan };
          writeClosedPlan = true;
        } else if (resumedAbsent) {
          closeResult = { kind: "ok"         , closedId: closePhaseId, plan: resumedAbsent };
          writeClosedPlan = true;
        } else {
          closeResult = { kind: "ok"         , closedId: closePhaseId, plan };
        }
      } else {
        ...
        if (plan.workPhases.every((workPhase) => workPhase.status === "done")) {
          ...
          return { output: "", advanced: null, allDone: true          };
        }
      ...
      return { output: "", advanced: closeResult, allDone: false          };
    });
 ...
 */
export function safeReadBoundGoalplan(cwd        , slug        )                                  {
 ...
 */
function observeProgress(cwd        , state       )                      {
...
function relevantPresentedNativeCriteria(plan          )                       {
...
function nativeSurfaceGroundingAdvisory(criterionIds          )         {
  return [
