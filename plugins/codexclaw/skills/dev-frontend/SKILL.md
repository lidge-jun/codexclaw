...
| `references/core/token-source-divergence.md` | Changing colour, type, spacing, or breakpoint values in a repo that declares the same tokens in two or more places | Source inventory, authority and sync arrangement, one-sided-change gate, literal classification, rendered parity |
| `references/core/content-surface-pipeline.md` | Speeding up or changing a surface produced repeatedly by a content-design-development chain; read alongside `layout-discipline.md` (layout grammar) and `asset-requirements.md` (asset production), which own different questions about the same pages | Mapping the handoff, locating the constraining step, multi-path render resolution, data-injected style, preview fidelity, per-block QA, release coupling |
