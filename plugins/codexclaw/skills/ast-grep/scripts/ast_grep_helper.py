...
# --- codexclaw runtime resolution ---

def codexclaw_env_binary() -> Optional[Path]:
    raw_path = os.environ.get("CODEXCLAW_AST_GREP_SG_PATH") or os.environ.get("OMO_AST_GREP_SG_PATH")
    if not raw_path:
        return None
...
def codexclaw_runtime_slug() -> str:
...
def codexclaw_runtime_binary() -> Optional[Path]:
    binary_name = "sg.exe" if sys.platform.startswith("win") else "sg"
    slug = codexclaw_runtime_slug()
    candidates: list[Path] = []

    codex_home = os.environ.get("CODEX_HOME")
    if codex_home:
        candidates.append(Path(codex_home) / "runtime" / "ast-grep" / slug / binary_name)
    candidates.append(Path.home() / ".codexclaw" / "runtime" / "ast-grep" / slug / binary_name)
...
def resolve_binary() -> Optional[Path]:
    ...
    1. CODEXCLAW_AST_GREP_SG_PATH override (OMO_AST_GREP_SG_PATH fallback)
    2. codexclaw runtime dirs
    3. Cached binary in <skill>/bin/
    4. PATH (via shutil.which)
    5. Homebrew default paths
    """
    for fn in (codexclaw_env_binary, codexclaw_runtime_binary, cached_binary, which_binary, homebrew_binary):
        result = fn()
        if result:
            return result
    return None
