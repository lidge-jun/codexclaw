...
- **Tool-surface discovery (DISPATCH-DISCOVER-01):** the default collab surface is V1 (`multi_agent_v1.*` namespace, deferred behind `tool_search`). When V2 is manually enabled, the tools are the flat multi_agent_v2 set — `spawn_agent` (task_name + message required, `fork_turns:"none"` for role dispatches) / `send_message` / `followup_task` / `wait_agent` / `interrupt_agent` / `list_agents` — exposed DIRECT. Always check: if `spawn_agent` is not visible, run `tool_search` for "spawn agent" FIRST; a V1 session shows `multi_agent_v1.*` (`send_input`/`resume_agent`/`close_agent`) — use those verbs; do not conclude dispatch is impossible (`structure/60_native_capabilities.md` §1).
...
- **Leaf topology (LEAF-TOPOLOGY-01, 260709):** subagents are star-topology LEAVES. V2 has NO spawn-depth limit upstream, so codexclaw enforces it: the `^spawn_agent$` hook DENIES any spawn issued by a subagent session (hook stdin carries `agent_id` only for thread-spawn children) unless the outgoing message contains `CXC-SUBSPAWN-ALLOWED`; every spawn message also gets a `[CXC-LEAF-GUARD]` constraint block, and a spawn with no explicit effort gets `reasoning_effort: high` injected so a child never inherits Ultra (which would re-arm Proactive delegation in the child). Grant recursion ONLY deliberately, per dispatch, by putting `CXC-SUBSPAWN-ALLOWED` in the spawn message — that also lifts the guard block for that coordinator child.
...
`$codexclaw:cxc-<skill>` when a link is unsafe. The child's first turn parses either form
and injects the full SKILL.md bodies. This works on BOTH spawn surfaces (`message` is a
shared field; v2's `deny_unknown_fields` only blocks the extra `items` key). The always-on
`^spawn_agent$` PreToolUse hook only normalizes known broken/bare cxc mentions; it does not
add role baselines or infer missing surfaces. Structured v1 `items` (via
`resolveSpawnPayloadWithSkills`, L15) remains the strongest form where available. Do not delegate
