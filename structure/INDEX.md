...
| `PreToolUse` spawn/collaboration variants | `hooks/pre-tool-use-attaching-skills.json` | `node "${PLUGIN_ROOT}/components/subagent-config/dist/spawn-attach-hook.js" hook pre-tool-use` | normalizes cxc mentions and inlines recognized skill bodies on V2-shaped spawns when the message reaches the hook as plaintext (native ChatGPT-backend V2 encrypts it — there only the guard and model/effort routing apply), and applies model+effort routing plus leaf guards on both surfaces; never adds missing skills |
